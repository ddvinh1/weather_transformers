...
YEARS = [str(x) for x in range(2003, 2004)]  # train from 2007-2017, validate on 2019, test on 2018.
...
PRESSURE_LEVEL = ['50', '100', '150',
                  ...
                  '700', '850', '925',
                  '1000']
AREA = [10.82, 106.62, 10.81, 106.63]
...
async def main():
    ...
    for years, root in zip([train_years, valid_years, test_years], [train_root, valid_root, test_root]):
        ...
        await asyncio.gather(*[download("reanalysis-era5-pressure-levels", UPPER_AIR_VARIABLES, year, month, DAYS, TIMES, join(root, f"upper_air_{year}_{month}.nc"), pressure_level=PRESSURE_LEVEL)for year in years for month in MONTHS])
