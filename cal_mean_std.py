...
def cal_mean(filenames, surface_variables, pLevel=None):
    """
    Calculate the mean of the given variables in the files.
    
    Args:
        filenames (list): List of filenames to calculate the mean from.
        surface_variables (list): List of variables to calculate the mean from.
        pLevel (int, optional): If given, calculate the mean for the given pressure level.
        
    Returns:
        dict or dict of dict: If pLevel is None, returns a dict with the mean of each variable.
                              If pLevel is given, returns a dict with the pressure level as key and
                              a dict with the mean of each variable as value.
    """
...
def cal_std(filenames, surface_variables, surface_mean, pLevel=None):
    """
    Calculate the standard deviation of the given variables in the files.
    
    Args:
        filenames (list): List of filenames to calculate the standard deviation from.
        surface_variables (list): List of variables to calculate the standard deviation from.
        surface_mean (dict): Dictionary with the mean of each variable.
        pLevel (int, optional): If given, calculate the standard deviation for the given pressure level.
        
    Returns:
        dict or dict of dict: If pLevel is None, returns a dict with the standard deviation of each variable.
                              If pLevel is given, returns a dict with the pressure level as key and
                              a dict with the standard deviation of each variable as value.
    """
    surface_map = {v: {"batch": 0, "sumV": 0} for v in surface_variables}
    for file in tqdm(filenames):
        ds = xr.open_dataset(file)
