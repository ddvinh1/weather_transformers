...
class EarthSpecificBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=None, shift_size=None, mlp_ratio=4.,
        ...
        # Calculate the padded resolution. The padding is calculated by adding the left and right padding values for each dimension.
        # The left padding is the right padding value and the right padding is the left padding value.
        # The top padding is the bottom padding value and the bottom padding is the top padding value.
        # The front padding is the back padding value and the back padding is the front padding value.
        pad_resolution = list(input_resolution)
        pad_resolution[0] += (padding[-1] + padding[-2])  # Add left and right padding values for pressure levels.
        pad_resolution[1] += (padding[2] + padding[3])  # Add left and right padding values for latitude.
        pad_resolution[2] += (padding[0] + padding[1])  # Add left and right padding values for longitude.
        ...
        # Dropout path is applied with a probability of drop_path. If drop_path is greater than 0, a DropPath layer is added. Otherwise, a nn.Identity() layer is added.
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        # Normalization layer is applied after the attention and MLP layers.
        self.norm2 = norm_layer(dim)
        # The MLP layer is a fully connected feed-forward neural network with a hidden layer with a dimension that is mlp_ratio times the input dimension. The output dimension is the same as the input dimension.
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)

        # If shift_pl, shift_lat, and shift_lon are all true, the block will roll along all three dimensions.
        # This is used to apply the shift window masking.
        shift_pl, shift_lat, shift_lon = self.shift_size
        self.roll = shift_pl and shift_lat and shift_lon

        # If the block is rolling, the attn_mask is created using the get_shift_window_mask function.
        # If the block is not rolling, the attn_mask is set to None.
        if self.roll:
            attn_mask = get_shift_window_mask(pad_resolution, window_size, shift_size)
        else:
            attn_mask = None

        # The attn_mask is stored in a buffer, so it does not take up any memory during the forward pass.
...
class Pangu_lite(nn.Module):
    ...
    def __init__(self, embed_dim=192, num_heads=(6, 12, 12, 6), window_size=(2, 6, 12)):
        ...
        self.patchembed2d = PatchEmbed2D(
            ...
            img_size=(1, 1),
            patch_size=(8, 8),
            in_chans=4 + 3,  # 4 for surface variables and 3 surface mask
            embed_dim=embed_dim,
        )
        self.patchembed3d = PatchEmbed3D(
            ...
            img_size=(13, 1, 1),
            patch_size=(2, 8, 8),
            in_chans=5,     # 5 upper-air variables
            embed_dim=embed_dim
        )

        self.layer1 = BasicLayer(
            dim=embed_dim,
            input_resolution=(8, 1, 1),
            depth=2,
            num_heads=num_heads[0],
            window_size=window_size,
            drop_path=drop_path[:2]
        )
        self.downsample = DownSample(in_dim=embed_dim, input_resolution=(8, 1, 1), output_resolution=(8, 1, 1))
        ...
        self.upsample = UpSample(embed_dim * 2, embed_dim, (8, 1, 1), (8, 1, 1))
        ...
        self.patchrecovery2d = PatchRecovery2D((1, 1), (8, 8), 2 * embed_dim, 4)
        self.patchrecovery3d = PatchRecovery3D((13, 1, 1), (2, 8, 8), 2 * embed_dim, 5)

    def forward(self, surface, surface_mask, upper_air, batch_size):
        """
        Args:
        ...
        if batch_size == 1:
            ...
            surface_mask = surface_mask.unsqueeze(0)  # Add the extra dimension
            surface_mask = surface_mask.expand(batch_size, -1, -1, -1)
